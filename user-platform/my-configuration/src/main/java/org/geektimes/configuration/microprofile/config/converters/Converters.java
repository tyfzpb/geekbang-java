...
public class Converters implements Iterable<Converter> {
    ...
    private final Map<Class<?>, PriorityQueue<PrioritisedConverter>> typedConverters = new HashMap<>();
    ...
    public void addConverter(Class<?> convertedType,Converter converter){
        addConverter(converter, DEFAULT_PRIORITY,convertedType);
    }
    ...
    public void addConverter(Converter converter, int priority, Class<?> convertedType) {
        ...
        priorityQueue.offer(new PrioritisedConverter(convertedType, priority,converter));
    }
    ...
    public List<Converter> getConverters(Class<?> convertedType) {
        PriorityQueue<PrioritisedConverter> prioritizedConverters = typedConverters.get(convertedType);
        if (prioritizedConverters == null || prioritizedConverters.isEmpty()) {
            return Collections.emptyList();
        }
        List<Converter> converters = new LinkedList<>();
        for (PrioritisedConverter prioritizedConverter : prioritizedConverters) {
    ...
    public Iterator<Converter> iterator() {
        ...
        for (PriorityQueue<PrioritisedConverter> converters : typedConverters.values()) {
            for (PrioritisedConverter converter : converters) {
